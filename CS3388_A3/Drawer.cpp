...
for (int i=0; i<p.getVertexList().size()-1; i++){
	Bresenham(p.getVertexList()[i].[1][1], p.getVertexList()[i].[2][1], p.getVertexList()[i].[1][1], p.getVertexList()[i].[2][1]);
}
//draw last line between final point and first point
//connect last point with first point
Bresenham(p.getVertexList()[p.getVertexList().size()-1].[1][1], p.getVertexList()[p.getVertexList().size()-1].[2][1], p.getVertexList()[0].[1][1], p.getVertexList()[0].[2][1]);
}